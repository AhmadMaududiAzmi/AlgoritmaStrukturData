...
public class DoubleLinkedList {
    ...
    public void addFirst(CD_Azmi m) {
        if (isEmpty()) {
    ...
    public void addLast(CD_Azmi item) {
        if (isEmpty()) {
            addFirst(item);
        } else {
            Node current = head;
    ...
    public void add(CD_Azmi item, int index) throws Exception {
        if (isEmpty()) {
            addFirst(item);
    ...
    public void print() {
        if (isEmpty()) {
            ...
            Node tmp = head;
            while (tmp != null) {
                System.out.print(tmp.data.kode + "\t" + tmp.data.judul + tmp.data.distributor + tmp.data.stok + tmp.data.rusak +"\n");
                tmp = tmp.next;
            }
            System.out.println("\nBerhasil diisi");
        }
    ...
    public int[][] addArr() {
        ...
        for (int i = 0; i < arr.length; i++) {
            if (tmp != null) {
                arr[i][0] = tmp.data.kode;
                arr[i][1] = "" + tmp.data.kode;
            }
            tmp = tmp.next;
        }
        return arr;
